<template>
  <div :class="['dropdown', { show: isShown }]">
    <button
      type="button"
      :class="[
        'button',
      ...
      @click="toggleShow"
    >
      <app-icon v-if="hasIcon" :icon="selected.icon" />
      {{ caption }}
    </button>

    <div :class="['dropdown__menu', { show: isShown }]">
      <button
...
export default {
  ...
  computed: {
    ...
    caption() {
      return `${this.title} ${this.value ? '- ' + this.selected.text : ''}`;
    },
    selected() {
      return (
...
.button.dropdown__toggle:after {
  ...
  right: 16px;
  transform: none;
  background: url('../../../public/assets/icons/icon-chevron-down.svg')
    no-repeat;
